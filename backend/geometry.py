...
the room's width, y -> away from the camera across its depth. So the quad's
near edge (closest to the camera, lowest in frame) is y = depth_cm and its far
edge is y = 0.
"""
...
class FloorProjection:
    ...
    def from_calibration(
        ...
        # side, which is the room's far edge in solver coordinates (y = depth).
        src = [
            (0.0, depth_cm),
            (width_cm, depth_cm),
            (width_cm, 0.0),
...
def selftest() -> None:
    ...
    # 1. Corners round-trip to the quad they were fitted from.
    for (x, y), expect in zip(
        [(0, D), (W, D), (W, 0), (0, 0)], camera.quad.as_list()
    ):
    ...
    near = proj.project(W / 2, D)
    far = proj.project(W / 2, 0)
    assert far[1] < near[1], "far edge should sit higher in the image"
    assert proj.scale_at(W / 2, 0) < proj.scale_at(W / 2, D), (
        "objects at the back should render smaller"
    )

    # 3. A footprint stays ordered and convex-ish: near edge wider than far.
    ...
    # 4. A bounding box lifts above the floor and contains the footprint.
    ...
    # 5. A degenerate quad is refused rather than silently fitted.
    ...
    print(f"near-edge scale: {proj.scale_at(W/2, D):.5f} img-units/cm")
